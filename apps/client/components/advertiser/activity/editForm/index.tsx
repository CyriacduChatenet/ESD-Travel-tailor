...
export const EditActivityForm: FC = () => {
    ...
    const handleScheduleDelete = async (id: string, index: number) => {
        await ActivityScheduleService.deleteActivitySchedule(`${process.env.NEXT_PUBLIC_API_URL}`, id, setApiErrors);
    ...
    const handleClosingDayDelete = async (id: string, index: number) => {
        await ActivityClosingDayService.deleteActivityClosingDay(`${process.env.NEXT_PUBLIC_API_URL}`, id, setApiErrors);
        setClosingDays(closingDays.filter((_, i) => i !== index))
    };

    const handleTagDelete = async (id: string, index: number) => {
        await ActivityTagService.deleteActivityTag(`${process.env.NEXT_PUBLIC_API_URL}`, id, setApiErrors);
        ...
        <div className="max-w-md mx-auto mt-4 col-span-4 md:col-span-8 xl:col-span-12">
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="mb-4">
                    ...
                    {tags.length === 0 && <p className="mt-2 text-red-500 text-xs italic">{errors.content?.message?.toString()}</p>}
                </div>
                <div className="mb-4">
                    <label htmlFor="Schedules" className="block text-gray-700 font-bold mb-2">
                        Schedules
                    </label>
                    <div className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        <div>
                            {schedules.map((schedule: ActivitySchedule, index: number) => (
                                <div key={index} className="flex justify-around items-center rounded-full bg-blue-500 text-white my-2">
                                    <p>{schedule.opening_at} - {schedule.closing_at}</p>
                                    <button
                                        className="text-white font-bold py-2 px-4"
                                        onClick={() => handleScheduleDelete(`${String(schedule.id)}`, index)}
                                    >
                                        <Icon icon="material-symbols:close-rounded" />
                                    </button>
                                </div>
                    ...
                    {schedules.length === 0 && <p className="mt-2 text-red-500 text-xs italic">{errors.opening_at?.message?.toString()} & {errors.closing_at?.message?.toString()}</p>}
                </div>
                <div className="mb-4">
                    <label htmlFor="Closing_Days" className="block text-gray-700 font-bold mb-2">
                        Closing days
                    </label>
                    <div className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        <div>
                            {closingDays.map((closingDay: ActivityClosingDay, index: number) => (
                                <div key={index} className="flex justify-around items-center rounded-full bg-blue-500 text-white my-2">
                                    <p>{new Date(closingDay.date).toLocaleDateString('fr')} - recurrence: {closingDay.recurrence ? 'true' : 'false'}</p>
                                    <button
                                        className="text-white font-bold py-2 px-4"
                                        onClick={() => handleClosingDayDelete(`${String(closingDay.id)}`, index)}
                                    >
                                        <Icon icon="material-symbols:close-rounded" />
                                    </button>
                                </div>
                    ...
                    {closingDays.length === 0 && <p className="mt-2 text-red-500 text-xs italic">{errors.recurrence?.message?.toString()} & {errors.date?.message?.toString()}</p>}
                </div>
                <button
                    type="submit"
