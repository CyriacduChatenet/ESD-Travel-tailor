...
const createTaste = (api_url: string, credentials: CreateTasteDTO) => {
    return useFetch.protectedPost(api_url, credentials, String(TokenService.getAccessToken()));
};

const updateTaste = (api_url: string, credentials: UpdateTasteDTO) => {
    return useFetch.protectedPatch(`${api_url}`, credentials, String(TokenService.getAccessToken()));
};
