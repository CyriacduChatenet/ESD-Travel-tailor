...
const findAllActivitySchedules = async (api_url: string, setError: Dispatch<SetStateAction<any>>): Promise<ActivitySchedule[]> => {
    const data = await useFetch.get(`${api_url}${API_ACTIVITY_SCHEDULE_ROUTE}`, setError)
    return data.data
};

const findActivityScheduleById = async (api_url: string, id: string, setError: Dispatch<SetStateAction<any>>): Promise<ActivitySchedule> => {
    return await useFetch.get(`${api_url}${API_ACTIVITY_SCHEDULE_ROUTE}/${id}`, setError)
};

const createActivitySchedule = async (api_url: string, credentials: CreateActivityScheduleDTO, setError: Dispatch<SetStateAction<any>>): Promise<ActivitySchedule> => {
    return await useFetch.post(`${api_url}${API_ACTIVITY_SCHEDULE_ROUTE}`, credentials, setError);
};

const updateActivitySchedule = async (api_url: string, id: string, credentials: UpdateActivityScheduleDTO, setError: Dispatch<SetStateAction<any>>): Promise<ActivitySchedule> => {
...
const deleteActivitySchedule = async (api_url: string, id: string, setError: Dispatch<SetStateAction<any>>) => {
