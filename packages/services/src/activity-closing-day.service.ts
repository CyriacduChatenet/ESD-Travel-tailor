...
const findAllActivityClosingDays = async (api_url: string, setError: Dispatch<SetStateAction<any>> | any): Promise<ActivityClosingDay[]> => {
    const data = await useFetch.get(`${api_url}${API_ACTIVITY_CLOSING_DAY_ROUTE}`, setError)
    return data.data;
};

const findActivityClosingDayById = async (api_url: string, id: string, setError: Dispatch<SetStateAction<any>> | any): Promise<ActivityClosingDay> => {
    return await useFetch.get(`${api_url}${API_ACTIVITY_CLOSING_DAY_ROUTE}/${id}`, setError)
};

const createActivityClosingDay = async (api_url: string, credentials: CreateActivityClosingDayDTO, setError: Dispatch<SetStateAction<any>> | any): Promise<ActivityClosingDay> => {
    return await useFetch.post(`${api_url}${API_ACTIVITY_CLOSING_DAY_ROUTE}`, { ...credentials, date: new Date(credentials.date)}, setError);
};

const updateActivityClosingDay = async (api_url: string, id: string, credentials: UpdateActivityClosingDayDTO, setError: Dispatch<SetStateAction<any>> | any): Promise<ActivityClosingDay> => {
...
const deleteActivityClosingDay = async (api_url: string, id: string, setError: Dispatch<SetStateAction<any>> | any) => {
