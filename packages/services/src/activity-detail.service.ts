...
const findAllActivityDetails = async (api_url: string, setError: Dispatch<SetStateAction<any>> | any):Promise<ActivityDetail[]> => {
    const data = await useFetch.get(`${api_url}${API_ACTIVITY_DETAIL_ROUTE}`, setError);
    return data.data
};

const findActivityDetailById = async (api_url: string, id: string, setError: Dispatch<SetStateAction<any>> | any): Promise<ActivityDetail> => {
    return await useFetch.get(`${api_url}${API_ACTIVITY_DETAIL_ROUTE}/${id}`, setError)
};

const createActivityDetail = async (api_url: string, credentials: CreateActivityDetailDTO, setError: Dispatch<SetStateAction<any>> | any): Promise<ActivityDetail> => {
    return await useFetch.post(`${api_url}${API_ACTIVITY_DETAIL_ROUTE}`, credentials, setError);
};

const updateActivityDetail = async (api_url: string, id: string, credentials: UpdateActivityDetailDTO, setError: Dispatch<SetStateAction<any>> | any): Promise<ActivityDetail> => {
...
const deleteActivityDetail = async (api_url: string, id: string, setError: Dispatch<SetStateAction<any>> | any) => {
