...
const findAllTastes = async (api_url: string, setError: Dispatch<SetStateAction<any>>): Promise<Taste[]> => {
  const data = await useFetch.get(`${api_url}${API_TASTE_ROUTE}`, setError)
  return data.data
}

const findOneTaste = async (api_url: string, id: string, setError: Dispatch<SetStateAction<any>>): Promise<Taste> => {
  return await useFetch.get(`${api_url}${API_TASTE_ROUTE}/${id}`, setError)
}

const createTaste = async (api_url: string, credentials: CreateTasteDTO, setError: Dispatch<SetStateAction<any>>): Promise<Taste> => {
...
const createTasteWithRelation = async (api_url: string, tastes: Taste[], travelerId: string, setError: Dispatch<SetStateAction<any>>) => {
...
const deleteTaste = async (api_url: string, id: string, setError: Dispatch<SetStateAction<any>>) => {
