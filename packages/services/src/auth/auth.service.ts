...
const forgotPassword = async (api_url: string, forgotPasswordCredentials: ForgotPasswordDTO) => {
	return await useFetch.post(api_url, forgotPasswordCredentials);
};

const resetPassword = async (api_url: string, resetPasswordCredentials: ResetPasswordDTO, resetToken: string) => {
	return await useFetch.protectedPost(`${api_url}/${resetToken}`, resetPasswordCredentials, resetToken);
};
