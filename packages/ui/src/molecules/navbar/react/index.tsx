...
export const WebNavbar: FC = () => {
  ...
  return (
    <nav className={styles.navbar}>
      ...
      {user.roles.length > 0 ? (
        <>
        {user.roles.includes(ROLES.TRAVELER) ? <Link href={'/traveler/dashboard'}>Traveler dashboard</Link> : null}
        {user.roles.includes(ROLES.ADVERTISER) ? <Link href={'/advertiser/dashboard'}>Advertiser dashboard</Link> : null}
        </>
      ) : null}
      <Link href={'/signin'}>
